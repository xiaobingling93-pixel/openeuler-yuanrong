...
- `values.cpu_num`：节点可用 CPU 总量，单位毫核（ 1/1000 核）。部署主节点时，系统默认占用 1 毫核。您可以在部署主节点时设置为 `-s 'values.cpu_num=1'`，则该节点不用于运行分布式任务。
- `values.memory_num`：节点可用内存总量，单位 MB。其中一部分用于存储"数据对象"，一部分用于函数的堆栈内存。
- `values.shared_memory_num`：节点可用内存总量中，用于存储"数据对象"的内存量，单位 MB。
- `function_agent.args.gpu_collection_enable`：自动探测节点上的 GPU 资源量。
- `function_agent.args.npu_collection_mode`：自动探测节点上的 NPU 资源。
- `function_agent.args.custom_resources`：除 GPU、NPU 外的其他自定义资源，例如 SSD。
