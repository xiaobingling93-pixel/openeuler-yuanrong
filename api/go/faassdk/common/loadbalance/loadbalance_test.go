...
func (lbs *LBTestSuite) SetupTest() {
	lbs.m = sync.RWMutex{}
	lbs.LBInterface = LBFactory(lbs.lbType)
}
...
func TestLimiterCHGeneric_Next(t *testing.T) {
	convey.Convey("TestLimiterCHGeneric_Next", t, func() {
		limiter := NewLimiterCHGeneric(1)
		next := limiter.Next("n1", false)
		convey.So(next, convey.ShouldEqual, "")
		limiter.Add("n1", 0)
		next = limiter.Next("n1", false)
		convey.So(next, convey.ShouldEqual, "n1")
		limiter.Add("n2", 0)
		next = limiter.Next("n1", true)
		convey.So(next, convey.ShouldEqual, "n2")
		limiter.Remove("n1")
		limiter.Remove("n2")
	})
}

func TestLimiterCHGeneric_SetStain(t *testing.T) {
	convey.Convey("TestLimiterCHGeneric_SetStain", t, func() {
		limiter := NewLimiterCHGeneric(1)
		limiter.Add("faasscheduelrID-1", 0)
		next := limiter.Next("test-function", false)
		convey.So(next, convey.ShouldEqual, "faasscheduelrID-1")

		limiter.SetStain("test-function", "faasscheduelrID-1")
		next = limiter.Next("test-function", false)
		convey.So(next, convey.ShouldEqual, "faasscheduelrID-1")
