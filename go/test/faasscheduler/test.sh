...
ROOT_PATH=${CUR_DIR}/../../
SRC_PATH=${ROOT_PATH}/pkg/functionscaler
CMD_PATH=${ROOT_PATH}/cmd/faas/faasscheduler
...
run_gocover_report()
{
    ...
    cd ${SRC_PATH}
    go test -gcflags=all=-l -covermode="${GOCOVER_MODE}" -coverprofile="$OUTPUT_PATH/faasscheduler.cover" -coverpkg="./..." "./..."

    if [ $? -ne 0 ]; then
        log_error "failed to go test faasscheduler"
        exit 1
    fi
