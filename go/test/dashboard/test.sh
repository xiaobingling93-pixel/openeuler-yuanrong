...
ROOT_PATH=${CUR_DIR}/../../
SRC_PATH=${ROOT_PATH}/pkg/dashboard
CMD_PATH=${ROOT_PATH}/cmd/dashboard
...
run_gocover_report()
{
    ...
    cd ${SRC_PATH}
    go test -gcflags=all=-l -covermode="${GOCOVER_MODE}" -coverprofile="$OUTPUT_PATH/dashboard.cover" -coverpkg="./..." "./..."

    if [ $? -ne 0 ]; then
        log_error "failed to go test dashboard"
        exit 1
    fi
