...
func GetAlarmLogger() (*zap.Logger, error) {
	createLoggerOnce.Do(func() {
		...
		alarmLogger = alarmLogger.With(zapcore.Field{
			...
			String: os.Getenv(constants.WiseCloudSite),
		}, zapcore.Field{
			Key: "tenant_id", Type: zapcore.StringType,
			String: os.Getenv(constants.TenantID),
		}, zapcore.Field{
			Key: "application_id", Type: zapcore.StringType,
			String: os.Getenv(constants.ApplicationID),
		}, zapcore.Field{
			Key: "service_id", Type: zapcore.StringType,
			String: os.Getenv(constants.ServiceID),
		})
	})
...
func SetAlarmEnv(alarmConfigInfo config.CoreInfo) {
	...
	log.GetLogger().Infof("succeeded to set env of %s, value: %s", ConfigKey, string(alarmConfigBytes))
}

// SetXiangYunFourConfigEnv -
func SetXiangYunFourConfigEnv(xiangYunFourConfig types.XiangYunFourConfig) {
	if err := os.Setenv(constants.WiseCloudSite, xiangYunFourConfig.Site); err != nil {
		log.GetLogger().Errorf("failed to set env of %s, err: %s", constants.WiseCloudSite, err.Error())
	}
	if err := os.Setenv(constants.TenantID, xiangYunFourConfig.TenantID); err != nil {
		log.GetLogger().Errorf("failed to set env of %s, err: %s", constants.TenantID, err.Error())
	}
	if err := os.Setenv(constants.ApplicationID, xiangYunFourConfig.ApplicationID); err != nil {
		log.GetLogger().Errorf("failed to set env of %s, err: %s", constants.ApplicationID, err.Error())
	}
	if err := os.Setenv(constants.ServiceID, xiangYunFourConfig.ServiceID); err != nil {
		log.GetLogger().Errorf("failed to set env of %s, err: %s", constants.ServiceID, err.Error())
	}
	log.GetLogger().Infof("succeeded to set env, value: %v", xiangYunFourConfig)
}
...
func SetPodIP() error {
	...
	err = os.Setenv(constants.PodIPEnvKey, ip)
	if err != nil {
		log.GetLogger().Errorf("failed to set env of %s, err: %s", constants.PodIPEnvKey, err.Error())
		return err
	}
	return nil
}
...
// SetClusterNameEnv -
func SetClusterNameEnv(clusterName string) {
	if err := os.Setenv(ClusterName, clusterName); err != nil {
		log.GetLogger().Errorf("failed to set env of %s, err: %s", ClusterName, err.Error())
	}
	log.GetLogger().Infof("succeeded to set env of %s, value: %s", ClusterName, clusterName)
}
