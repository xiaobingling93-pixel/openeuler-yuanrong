...
.. cpp:function:: void YR::SaveState(const int &timeout = DEFAULT_SAVE_LOAD_STATE_TIMEOUT)
    ...
    抛出：
        ...
        - 本地模式不支持 SaveState，将抛出异常：“SaveState is not supported in local mode”。
        - 集群模式仅在远程代码中支持此操作；如果在本地代码中调用，将抛出异常：“SaveState is only supported on cloud with posix api”。
