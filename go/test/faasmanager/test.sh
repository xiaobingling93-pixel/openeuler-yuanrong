...
ROOT_PATH=${CUR_DIR}/../../
SRC_PATH=${ROOT_PATH}/pkg/functionmanager
...
run_gocover_report()
{
    ...
    cd ${SRC_PATH}
    go test -gcflags=all=-l -covermode="${GOCOVER_MODE}" -coverprofile="$OUTPUT_PATH/faasmanager.cover" -coverpkg="./..." "./..."

    if [ $? -ne 0 ]; then
        log_error "failed to go test faasmanager"
        exit 1
    fi
